...
void Test(string_view mark, const SearchServer &search_server,
...
int main() {
  ...
  for (const string &text : {
           ...
           "nasty pigeon john"s,
           "cringy dog cat meme"s,
       }) {
    search_server1.AddDocument(++id, text, DocumentStatus::ACTUAL, {1, 2});
  }

  cout << "ACTUAL by default:"s << endl << "(seq)" << endl;
  // последовательная версия
  for (const Document &document :
       search_server1.FindTopDocuments("curly nasty cat"s)) {
    PrintDocument(document);
  }
  cout << "(par)"s << endl;
  for (const Document &document :
       search_server1.FindTopDocuments(execution::par, "curly nasty cat"s)) {
    PrintDocument(document);
  }

  cout << "BANNED:"s << endl << "(seq)" << endl;
  // последовательная версия
  ...
  // последовательная версия
  for (const Document &document : search_server1.FindTopDocuments(
           execution::par, "curly nasty cat"s, DocumentStatus::BANNED)) {
    PrintDocument(document);
  }

  cout << "Even ids:"s << endl << "(seq)" << endl;
  for (const Document &document : search_server1.FindTopDocuments(
