...
void AddDocument(SearchServer &search_server, int document_id,
    ...
    try {
        ...
        cout << "Error occurred while adding document "s << document_id << ": "s << e.what()
             << endl;
    }
}
...
void TestMatching() {
    auto result =
            TEST_SERVER.MatchDocument("fluffy groomed cat dog -dinner"s, 1);
...
void TestStatus() {
    size_t banned = TEST_SERVER
            .FindTopDocuments("fluffy hippo cat"s,
